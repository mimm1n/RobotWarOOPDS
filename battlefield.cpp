...
// Tutorial Class: TT7L
// Trimester: 2510
// Member_1: 243UC24665 | NOR ALIYAH SYAHIRAH BINTI MUHD HASSANAL |
// NOR.ALIYAH.SYAHIRAH@student.mmu.edu.my | 0146202605 Member_2: 243UC246KQ |
// KHAYRIN SOFIYA BINTI JAMEL | KHAYRIN.SOFIYA.JAMEL@student.mmu.edu.my |
// 0193320041 Member_3: 243UC247CJ | AIMI BINTI MOHD FAIZAL |
// AIMI.MOHD.FAIZAL@student.mmu.edu.my | 0139071648 Member_4: 241UC24199 |
// AMIRUL IHTISYAM BIN IDRUS | AMIRUL.IHTISYAM.IDRUS@student.mmu.edu.my |
// 0194090095
//*********************************************************
// Task Distribution
// Member_1:
...
void Battlefield::readFile(string filename) {
  ...
  iss >> battlefieldRows_ >>
      battlefieldCols_; // stores the matrix to columns and rows
...
void Battlefield::placeRobots() {
  ...
  for (int i = 0; i < robots_.size(); ++i) {
    if (robots_[i]->getRobotY() < battlefield_.size() &&
        robots_[i]->getRobotX() < battlefield_[0].size()) {
      battlefield_[robots_[i]->getRobotY()][robots_[i]->getRobotX()] =
          robots_[i]->getRobotName();
    } else {
      cout << "Error message: Invalid location for the robot "
           << robots_[i]->getRobotName() << endl;
      exit(1);
    }
  }
}
